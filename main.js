...
var roleRoomGuard = require('role.roomGuard');
...
module.exports.loop = function () {
    ...
    for (let spawnName in Game.spawns) {
        ...
        */
        ...
        if (spawn.canCreateCreep([MOVE], undefined) == OK) {
            ...
            var numberOfRoomGuards = _.sum(creepsInRoom, (c) => c.memory.role == 'roomGuard' &&
                                                                c.ticksToLive > spawnBeforeTicksHomeRoom);
            ...
            if (containers < 4) {
                ...
                } else if (numberOfRoomGuards < spawn.memory.minNumberOfRoomGuards) {
                    //Game.spawns[spawn.name].createBuilder(maximumAvailableEnergy);
                }
            }
            ...
            if (attackRooms) {
                ...
                for (let targetRoom of targetAttackRooms) {
                    ...
                    if (numberOfLongDistanceAttackers < minimumNumberOfAttackers) {
                        Game.spawns[spawn.name].createCreep([MOVE, MOVE, ATTACK, ATTACK],{role: 'attacker'});
                        /*Game.spawns[spawn.name].createCreep([TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,
                                                             MOVE,MOVE,MOVE,MOVE,MOVE,
                                                             TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,
                                                             MOVE,MOVE,MOVE,MOVE,MOVE],{role: 'attacker'});*/
            ...
            if (maximumAvailableEnergy >= 700 &&
                    ...
                    numberOfTransporters >= spawn.memory.minNumberOfTransporters &&
                    numberOfEnergySmoothers >= spawn.memory.minNumberOfEnergySmoothers &&
                    numberOfUpgraders >= spawn.memory.minNumberOfUpgraders) {
                ...
                if (longDistanceRooms) {
                    let targetRooms = longDistanceRooms.split(",");
                        ...
                        for (let targetRoom of targetRooms) {
                            /*if (!Game.rooms[targetRoom].memory.sourceCount) {
                                countOfSources = Game.rooms[targetRoom].find(FIND_SOURCES).length;
                                Game.rooms[targetRoom].memory.sourceCount = countOfSources;
                            }*/
                            ...
                            if (numberOfRoomExplorers < spawn.memory.minNumberOfRoomExplorers) {
                                ...
                                Game.spawns[spawn.name].createlongDistanceMiner(maximumAvailableEnergy,
                                        ...
                                        Game.spawns[spawn.name].createCreep([MOVE, MOVE, MOVE, CARRY, CARRY, CARRY, WORK],
                    ...
                    for (let claimRoom of claimingRooms) {
                        ...
                        if (numberOfClaimers < 0) {
                            Game.spawns[spawn.name].createCreep([CLAIM, MOVE],
                                                                ...
                                                                roomClaimer: true});
                        } else if (numberOfLongDistanceHelpers < 0) {
